# giftr — complete context document

This document is a self-contained description of the R package **giftr**,
intended as the primary source of information for an AI assistant reasoning
about the project. It states what giftr is, what it deliberately is not, how
...
giftr deliberately occupies the space between **marker checklists** (KEGG
...
per bundle. giftr is a redesign around a different primitive, not a version
...
`alberdilab/giftr`. Dependencies: DBI, RSQLite, tibble; R >= 4.1.0.
...
## 3. What giftr deliberately does not model

Unless an explicit architectural decision changes scope, giftr does not model:
...
KEGG reaction IDs and EC numbers go in `reaction_xrefs.tsv`. giftr may query
...
human-reviewable TSV sources   (inst/extdata/database-source/*.tsv)
        |
        v  validate_giftr_sources()
structural validation
        |
        v  build_giftr_database()
SQLite compilation + foreign-key and integrity checks
        |
        v
read-only runtime queries and evaluation   (inst/extdata/giftr.sqlite)
```

The TSVs are the biological source of truth. **`inst/extdata/giftr.sqlite` is a
...
`inst/schema/giftr.sql`; the compiler and validator are `R/database-build.R`.
...
KEGG M00018 is `subset_of` for three different GIFTs because giftr cuts it at
...
support enzyme/marker mappings; **giftr chooses the capability, boundaries,
accepted routes, and curation interpretation.** Never imply that an upstream
database endorses a giftr-specific boundary. Dataset-level sources live in
...
```text
giftr package version     code and public API release      (0.1.0)
giftr database version    biological content release       (2026.12.1)
...
```r
giftr_db_connect(path = NULL, read_only = TRUE)
giftr_db_disconnect(db)
giftr_db_version(db = NULL)      # package, database and schema versions
```
...
build_giftr_database(source_dir, output, overwrite = FALSE, source_commit = NULL)
write_giftr_database_html(path, open = TRUE)
```
...
`evaluate_gifts()` returns an S3 `giftr_result` list of tibbles, all of which
...
`evaluate_reactions()` returns a `giftr_reaction_result` — the same machinery
stopped below the route layer, for use when reaction support rather than trait
support is the question.

### The HTML atlas

`write_giftr_database_html()` writes a single self-contained interactive page
...
`inst/extdata/giftr-database.html` and is refreshed by the build script.
...
about giftr should treat them as constraints, not preferences.
...
12. Preserve provenance, and distinguish imported facts from giftr's own
...
3. **Curate reaction routes** — map to Rhea master reactions, determine
   ...
   chemistry, alternatives and direction against the giftr claim.
...
├── inst/
│   ├── schema/giftr.sql           the relational contract (18 tables, 2 views)
│   ├── extdata/database-source/   the 18 hand-curated TSVs — the source of truth
│   ├── extdata/giftr.sqlite       compiled artifact (never hand-edited)
│   ├── extdata/giftr-database.html packaged atlas snapshot
