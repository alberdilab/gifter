...
test_that("a reaction without a Rhea master needs an external cross-reference", {
  ...
  expect_true(validate_gifter_sources(identified)$valid)

  unidentified <- gifter_source_copy()
...
test_that("a recorded Rhea master must be well formed and unique", {
  ...
  expect_error(validate_gifter_sources(duplicated), "Duplicated Rhea master IDs")
})
